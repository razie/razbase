...
class DslCollectors[A] {
  ...
  def simpleCollect[T](who: Any)(f: => T): List[A] = {
    val col = new collection.mutable.ListBuffer[A]()
    collect(col append _)(who)(f)
    col.toList
  }
...
/** mix-in trait for collectable units - collects itself in constructor 
 * 
 * you don't need to mix this in - can just copy the statement here
 */
trait DslCollectable { 
