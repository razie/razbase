 ...
 * @author razvanc99
 */
class GRefTest extends JUnit3Suite {
   val k1 = GRef ("Meta", "id")
...
//   val s5 = "razie.uri:Meta:id@http://gigi.com::c:\\Video@@someattr=somevalue&ctx.name="
   ...
   def testk1() = expect (k1) { GRef.parse(s1) }
   //def testk1a() = expect (k1a) { GRef.parse(s1a) }
   def testk2() = expect (k2) { GRef.parse(s2) }
   def testk3() = expect (k3) { GRef.parse(s3) }
   def testk4() = expect (k4) { GRef.parse(s4) }
   def testk4old() = expect (k4) { GRef.parse(s4old) }

   def testk1s() = expect (s1) { k1.toString }
   def testk2s() = expect (s2) { k2.toString }
   def testk3s() = expect (s3) { k3.toString }
   def testk4s() = expect (s4) { k4.toString }
  
   def testk7() = expect (k7) { GRef.parse("razie.gidref:Meta:id@c:\\Video") }
   def testk8() = expect (k8) { GRef.parse("razie.gaqref:Meta:name%3Djohn@http://gigi.com::c:\\Video") }
//   def testk9() = expect (k9) { GRef.parse("razie.gaqref:Meta:name%3Djohn@http://gigi.com::c:\Video") }
   def testk10() = expect (k10) { GRef.parse("razie.gxqref:Meta:%2FMeta%5B%40name%3D%3D%27john%27%5D@http://gigi.com::c:\\Video") }
//   def testZ() = expect (1) { k4.toString; println (k7); println (k8); println (k10); 1 }
